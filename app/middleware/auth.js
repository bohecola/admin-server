...
const auth = () => {
  return async (ctx, next) => {
    ...
    const whiteList = ['/page', '/info', '/tags', '/categories', '/records', '/profile'].map(path => `/api/posts${path}`);

    const openApis = ['/login', '/refreshToken', '/eps', '/captcha', '/qrcode', '/qrcodePoll'].map(path => `/api/admin/open${path}`);
